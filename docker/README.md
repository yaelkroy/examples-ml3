...
```bash
$ docker pull yaelkroy/examples-ml3
```

**Note**: this is the CPU-only image. For GPU support, read the GPU section below.

Alternatively, you can build the image yourself. This will be slower, but it will ensure the image is up to date, with the latest libraries. For this, assuming you already downloaded this project into the directory `/path/to/project/examples-ml3`:

```bash
$ cd /path/to/project/examples-ml3/docker
$ docker-compose build
```

This will take quite a while, but is only required once.

After the process is finished you have an `yaelkroy/examples-ml3:latest` image, that will be the base for your experiments. You can confirm that by running the following command:

```bash
$ docker images
REPOSITORY            TAG         IMAGE ID            CREATED             SIZE
yaelkroy/examples-ml3    latest      3ebafebc604a        2 minutes ago       4.87GB
```

### Run the notebooks

Still assuming you already downloaded this project into the directory `/path/to/project/examples-ml3`, run the following commands to start the Jupyter server inside the container, which is named `examples-ml3`:
...
Run `make exec` (or `docker-compose exec examples-ml3 bash`) while the server is running to run an additional `bash` shell inside the `examples-ml3` container. Now you're inside the environment prepared within the image.
...
* Replace `image: yaelkroy/examples-ml3:latest` with `image: yaelkroy/examples-ml3:latest-gpu`
...
```bash
$ docker pull yaelkroy/examples-ml3:latest-gpu
```
...
```bash
$ cd /path/to/project/examples-ml3
$ docker run --name examples-ml3 --gpus all -p 8888:8888 -p 6006:6006 --log-opt mode=non-blocking --log-opt max-buffer-size=50m -v `pwd`:/home/devel/examples-ml3 yaelkroy/examples-ml3:latest-gpu /opt/conda/envs/homl3/bin/jupyter notebook --ip='0.0.0.0' --port=8888 --no-browser
```
