...
If you do not want to install git, you can instead download [main.zip](https://github.com/yaelkroy/examples-ml3/archive/main.zip), unzip it, rename the resulting directory to `examples-ml3` and move it to your development directory.
...
Next, make sure you're in the `examples-ml3` directory and run the following command. It will create a new `conda` environment containing every library you will need to run all the notebooks (by default, the environment will be named `homl3`, but you can choose another name using the `-n` option):
...
For this, open a terminal, and run:
    ...
    $ cd $HOME # or whatever development directory you chose earlier
    $ cd examples-ml3 # go to this project's directory
    $ git pull
